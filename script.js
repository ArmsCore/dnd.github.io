...
function fun3() {   
    ...
    // Проверка вопроса 1 (2000)
    let resultQuestion1 = `Вопрос 1: ${question1 || "Не указан"}`;
    if (question1.toLowerCase() === "2000") {  // Проверка с использованием toLowerCase()
        resultQuestion1 += `<p style="color: green;">Правильно!</p>`;
        score++;  // Увеличиваем баллы за правильный ответ
    } else {
        resultQuestion1 += `<p style="color: red;">Правильный ответ: 2000</p>`;
    }
    document.getElementById("questionResult1").innerHTML = resultQuestion1;

    // Проверка вопроса 2 (Мэттью)
    let resultQuestion2 = `Вопрос 2: ${question2 || "Не указан"}`;
	// Проверяем, что question2 существует и не пустое
	if (question2 && question2.toLowerCase() === "мэттью мерсер") {
		resultQuestion2 += `<p style="color: green;">Правильно!</p>`;
		score++;  // Увеличиваем баллы за правильный ответ
	} else {
		resultQuestion2 += `<p style="color: red;">Правильный ответ: Мэттью</p>`;
	}
    ...
    if (question3 && question3.value == "Создать и прожить историю, взаимодействуя с миром.") {
        result3 += `<p style="color: green;">Правильно!</p>`;
        score++;  // Увеличиваем баллы за правильный ответ
    } else {
        result3 += `<p style="color: red;">Правильный ответ: Создать и прожить историю, взаимодействуя с миром.</p>`;
    }
    ...
    if (question4 && question4.value === "Следит за соблюдением правил и играет монстров.") {
        result4 += `<p style="color: green;">Правильно!</p>`;
        score++;  // Увеличиваем баллы за правильный ответ
    } else {
        result4 += `<p style="color: red;">Правильный ответ: Следит за соблюдением правил и играет монстров.</p>`;
    }
    ...
    if (question5 && question5.value == "D20 (двадцатигранник).") {
        result5 += `<p style="color: green;">Правильно!</p>`;
        score++;  // Увеличиваем баллы за правильный ответ
    } else {
        result5 += `<p style="color: red;">Правильный ответ: D20 (двадцатигранник).</p>`;
    }
    ...
    if (question6 && question6.value === "Количество урона, которое может выдержать персонаж.") {
        result6 += `<p style="color: green;">Правильно!</p>`;
        score++;  // Увеличиваем баллы за правильный ответ
    } else {
        result6 += `<p style="color: red;">Правильный ответ: Количество урона, которое может выдержать персонаж.</p>`;
    }
...
// Добавляем обработчик события для всех элементов с классом "term"
...
terms.forEach(term => {
    term.addEventListener('click', function() {
        ...
        // Вызываем функцию для отображения описания
        showDescription(this.textContent);
    });
});

// Словарь терминов и их описания
const dictionary = {
    "Реакция": "Реакция — это мгновенный ответ на срабатывание некоего условия (тригера), который может происходить как в ваш, так и в чужой ход.",
    "Хиты": "Хиты отражают комбинацию физической и умственной стойкости, стремление к существованию и удачу. Существ с большим числом хитов трудно убить. Те, у кого хитов мало — менее жизнеспособны.",
    "Происхождение": "Обычно первый шаг в создании истории жизни вашего персонажа – это определение ранних обстоятельств. Кем были ваши родители? Где вы родились? У вас были братья и сёстры? Кто вас воспитывал? Вы можете разрешить эти вопросы, используя следующие таблицы.",
    "Артефакты": "Артефакт это уникальный магический предмет потрясающей силы, со своим собственным происхождением и историей.",
    "Класс Доспеха": "Класс доспеха это характеристика отражающая способность персонажа или монстра избегать попаданий оружием во время боя.",
    "Спасброски": "Спасброски отражают вашу попытку сопротивляться заклинанию, ловушке, яду, болезни или другой подобной угрозе.",
    "Характеристики": "Характеристики персонажа — это перечень основных и дополнительных параметров, а также навыков и умений, которыми обладает игровой персонаж. ",
    "Бонусное действие": "Разнообразные классовые умения, заклинания и прочие эффекты позволяют вам совершать в ход дополнительное бонусное действие.",
    "Опыт": "Когда ваш персонаж отправляется в приключение и преодолевает испытания, он получает опыт, выраженный в виде очков опыта.",
    "Состояния": "Состояния это эффекты, которые по-разному изменяют способности персонажей."
  };
